...
struct BooksAPI {
    ...
    func popularBooksWest() async throws -> [Book] {
        let url = URL(string: "https://www.googleapis.com/books/v1/volumes?q=most+popular+young+adult+novels+2023")!
    ...
    func lightNovels() async throws -> [Book] {
        let url = URL(string: "https://www.googleapis.com/books/v1/volumes?q=best+light+novels")!
        let urlRequest = URLRequest(url: url)
        let (data, _) = try await urlSession.data(for: urlRequest)
